...
#include "TextureManager.h"  // skybox cubemap (Textures/Skybox.dds)
...
namespace Neuron::Graphics
{
  namespace
  {
    ...
    // Mirrors SkyboxCb in shaders/partials/skybox.hlsli (projection + camera->world rotation).
    struct SkyboxParams
    {
      float params[4]; // x = tan(halfFovY), y = aspect (w/h), z/w unused
      float rot0[4];   // camera->world rotation, row 0 (xyz)
      float rot1[4];   // row 1
      float rot2[4];   // row 2
    };
  ...
  void Scene3D::Shutdown()
  {
    ...
    s_skyCb = nullptr;
    s_skyDepth = nullptr;
    s_skySampler = nullptr;
    s_skyCubeSrv = nullptr;
    s_dustVs = nullptr;
    s_dustPs = nullptr;
  ...
  bool Scene3D::EnsureResources()
  {
    ...
    // Cube skybox: full-screen-triangle VS (no input layout / vertex buffer) + PS that samples
    // the environment cubemap by view direction, its b0 params, and a depth-disabled state (it
    // fills the background without touching the depth buffer, so the depth-tested ships draw
    // over it).
    ...
    // Cube sampler (linear; clamp - the hardware handles cross-face seams). Load the cubemap
    // itself; if it is missing / not a six-face cube, s_skyCubeSrv stays null and renderSkybox
    // simply skips (background stays the black clear).
    ...
    check_hresult(device->CreateSamplerState(&ssd, s_skySampler.put()));

    if (auto cube = TextureManager::LoadCubemap(kSkyboxCubemap); cube && cube->IsLoaded())
      s_skyCubeSrv.copy_from(cube->GetShaderResourceView());
  ...
  void Scene3D::renderSkybox()
  {
    ...
    if (!ctx || !s_skyCubeSrv || !s_skySampler)
      return; // no cubemap -> leave the black clear
    ...
    // The VS reads u_Params (tanY/aspect); the PS reads u_Rot* and samples the cube. Bind the
    // b0 cbuffer to both stages, plus the cube SRV + sampler to the PS.
    ID3D11Buffer* cb = s_skyCb.get();
    ctx->VSSetConstantBuffers(0, 1, &cb);
    ctx->PSSetConstantBuffers(0, 1, &cb);
    ID3D11ShaderResourceView* srv = s_skyCubeSrv.get();
    ID3D11SamplerState* samp = s_skySampler.get();
    ctx->PSSetShaderResources(0, 1, &srv);
    ctx->PSSetSamplers(0, 1, &samp);
    ctx->Draw(3, 0);

    // Unbind the cube SRV so it can't linger on t0 when a later pass binds t0 as an output.
