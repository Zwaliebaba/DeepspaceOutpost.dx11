...
Keep development as native as possible. **Do not develop wrapper functions or wrapper classes**
that merely forward to, rename, or thinly adapt an existing platform, SDK, or library API. Call
the native API directly.

A "wrapper" in this sense is any function or class whose only job is to pass a call through to
something that already exists — it adds no new behavior, no invariant, and no real abstraction.
...
- Prefer standard-library containers, algorithms, and smart pointers directly over custom
  equivalents (`Darray`, `Llist`, `FastDarray`, …) in new code.

A new class or function is justified only when it adds genuine behavior, an invariant, or a real
abstraction boundary — never as a thin pass-through. When in doubt, call the native API. When you
touch legacy wrapper code, prefer migrating it toward the native equivalent rather than adding
...
2. Use native APIs directly — do not add wrapper functions or classes around them (see *Native-First*).
3. Keep behavior in the module that owns it.
4. Update related interfaces, tests, and documentation together.
5. Preserve backward compatibility unless the change intentionally breaks it and is documented.
6. Prefer small, observable changes that can be reviewed and tested independently.
...
- [ ] No wrapper functions or classes were added around existing native APIs (Native-First).
