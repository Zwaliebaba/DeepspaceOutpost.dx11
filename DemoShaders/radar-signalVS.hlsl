...
struct VSInput
{
    ...
    float2 aTexCoord0 : TEXCOORD0;  // location 1 - inner
    float2 aTexCoord1 : TEXCOORD1;  // location 2 - outer
    float4 aColor     : COLOR0;     // location 3
    float  aRadius    : TEXCOORD2;  // location 4
};
...
#if (ENABLE_CLIP_PLANE0 || ENABLE_CLIP_PLANE1)
    o.clipDist = vclipping(input.aPos);
#endif

    o.v_TexCoord0 = input.aTexCoord0 + float2(-u_ShaderTime / input.aRadius, 0.0f);
    o.v_TexCoord1 = input.aTexCoord1 + float2(-u_ShaderTime, 0.0f);
