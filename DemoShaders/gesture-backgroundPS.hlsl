...
float4 PSMain(PSInput input) : SV_Target
{
    ...
    // Use alpha mask for gesture drawing area
    float4 alphaMask = float4(1.0, 1.0, 1.0, u_Texture1.Sample(u_Texture1_sampler, 1.0 - input.v_GradientCoord).r);

    // Mix the colors together
    float4 finalColor = float4(gradientColor + texColor, 1.0) * alphaMask;
