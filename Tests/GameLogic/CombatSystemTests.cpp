...
TEST(Missile, DestroysTheLockedTargetAhead)
{
  ECS::Registry w;
  ECS::EntityId shooter = SpawnShooter(w, GameLogic::Team::Player, /*laser*/ 1);
  // A healthy pirate dead ahead that a laser pulse could not one-shot.
  ECS::EntityId target = SpawnTargetAt(w, 0, 0, 4000, GameLogic::Team::Pirate, /*energy*/ 120);

  GameLogic::FireOutcome o = GameLogic::ResolvePlayerMissile(w, shooter, 20000, 0.85);
  EXPECT_TRUE(o.hit);
  EXPECT_TRUE(o.target == target);
  EXPECT_TRUE(o.targetTeam == GameLogic::Team::Pirate);
  EXPECT_TRUE(o.destroyed);   // MISSILE_DAMAGE (250) > 120
}

TEST(Missile, ReachesFartherThanTheLaser)
{
  ECS::Registry w;
  ECS::EntityId shooter = SpawnShooter(w, GameLogic::Team::Player, 20);
  // Beyond the 6000 laser reach but inside the missile's 20000 lock range.
  SpawnTargetAt(w, 0, 0, 12000, GameLogic::Team::Pirate, 120);

  EXPECT_TRUE(!GameLogic::ResolvePlayerFire(w, shooter, 6000, 0.9).hit);
  EXPECT_TRUE(GameLogic::ResolvePlayerMissile(w, shooter, 20000, 0.85).hit);
}

TEST(Missile, DoesNotDestroyTheStation)
{
  ECS::Registry w;
  ECS::EntityId shooter = SpawnShooter(w, GameLogic::Team::Player, 20);
  ECS::EntityId station = SpawnTargetAt(w, 0, 0, 3000, GameLogic::Team::Station, /*energy*/ 1000000);

  GameLogic::FireOutcome o = GameLogic::ResolvePlayerMissile(w, shooter, 20000, 0.85);
  EXPECT_TRUE(o.hit);
  EXPECT_TRUE(o.targetTeam == GameLogic::Team::Station);   // a crime...
  EXPECT_TRUE(!o.destroyed);                               // ...but the station shrugs it off
  EXPECT_TRUE(w.Get<GameLogic::Combatant>(station).energy == 1000000 - GameLogic::MISSILE_DAMAGE);
}
