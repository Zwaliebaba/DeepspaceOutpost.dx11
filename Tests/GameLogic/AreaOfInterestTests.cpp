...
TEST(Aoi, OnlyIncludesEntitiesNearTheViewer)
{
  ECS::Registry world;
  ...
  EXPECT_TRUE(Contains(snap, here.index));
  EXPECT_TRUE(Contains(snap, adjacent.index));
  EXPECT_TRUE(!Contains(snap, far.index));
  EXPECT_TRUE(snap.entities.size() == 2);
  EXPECT_TRUE(snap.tick == 1);
}

TEST(Aoi, RadiusZeroIsJustTheViewerCell)
{
  ECS::Registry world;
  ...
  EXPECT_TRUE(Contains(snap, a.index));
  EXPECT_TRUE(!Contains(snap, b.index));
  EXPECT_TRUE(snap.entities.size() == 1);
}

TEST(Aoi, CarriesComponentDataForVisibleEntities)
{
  ECS::Registry world;
  ...
  EXPECT_TRUE(snap.entities.size() == 1);
  EXPECT_TRUE(snap.entities[0].x == 5);
  EXPECT_TRUE(snap.entities[0].speed == 7.0f);
}

TEST(Aoi, RebuildReflectsMovement)
{
  ECS::Registry world;
  ...
  GameLogic::AreaOfInterest aoi(100);
  aoi.Rebuild(world);
  EXPECT_TRUE(Contains(aoi.SnapshotFor(world, 1, Math::Vector3i64{ 0, 0, 0 }, 0), mover.index));
  ...
  aoi.Rebuild(world);
  EXPECT_TRUE(!Contains(aoi.SnapshotFor(world, 2, Math::Vector3i64{ 0, 0, 0 }, 0), mover.index));
}

TEST(Despawn, ReportsEntitiesThatVanished)
{
  ...
  EXPECT_TRUE(tracker.TrackedCount() == 3);

  // Entity 2 disappears.
  gone = tracker.Update({ 1, 3 });
  EXPECT_TRUE(gone.size() == 1);
  EXPECT_TRUE(gone[0] == 2);
  ...
  EXPECT_TRUE(gone[0] == 1);
  EXPECT_TRUE(gone[1] == 3);
  EXPECT_TRUE(tracker.TrackedCount() == 0);
}

TEST(Despawn, NewEntitiesAreNotDespawns)
{
  GameLogic::DespawnTracker tracker;
  tracker.Update({ 1 });
  ...
  EXPECT_TRUE(gone.empty());
  EXPECT_TRUE(tracker.TrackedCount() == 2);
}
