...
TEST(Grid, CellOfHandlesNegativesAtBoundaries)
{
  Grid g(100);
  EXPECT_TRUE(g.CellSize() == 100);

  EXPECT_TRUE((g.CellOf(Vector3i64{ 0, 0, 0 }) == Cell{ 0, 0, 0 }));
  EXPECT_TRUE((g.CellOf(Vector3i64{ 99, 0, 0 }) == Cell{ 0, 0, 0 }));
  EXPECT_TRUE((g.CellOf(Vector3i64{ 100, 0, 0 }) == Cell{ 1, 0, 0 }));
  EXPECT_TRUE((g.CellOf(Vector3i64{ -1, 0, 0 }) == Cell{ -1, 0, 0 }));
  EXPECT_TRUE((g.CellOf(Vector3i64{ -100, 0, 0 }) == Cell{ -1, 0, 0 }));
  EXPECT_TRUE((g.CellOf(Vector3i64{ -101, 0, 0 }) == Cell{ -2, 0, 0 }));
}

TEST(Grid, QueryNearFindsByRadius)
{
  Grid g(100);
...
TEST(Grid, MoveRebucketsEntity)
{
  ...
  EXPECT_TRUE(atFrom.empty());                  // no longer in the old cell

  std::vector<uint64_t> atTo;
  g.QueryNear(to, 0, atTo);
  EXPECT_TRUE(atTo.size() == 1 && atTo[0] == 1);
  ...
  EXPECT_TRUE(still.size() == 1 && still[0] == 1);
}

TEST(Grid, RemoveEmptiesCell)
{
  Grid g(100);
  const Vector3i64 p{ 50, 50, 50 };
  g.Insert(1, p);
  g.Insert(2, p);
  EXPECT_TRUE(g.OccupiedCellCount() == 1);

  g.Remove(1, p);
  std::vector<uint64_t> after;
  g.QueryNear(p, 0, after);
  EXPECT_TRUE(after.size() == 1 && after[0] == 2);
  EXPECT_TRUE(g.OccupiedCellCount() == 1);
